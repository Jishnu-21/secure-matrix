...
const StatItem = ({ endNumber, text, prefix = '', suffix = '', isYear = false }: StatItemProps) => {
  ...
  return (
    <div ref={elementRef} className="flex flex-col items-center space-y-1 sm:space-y-2">
      <p className="text-[#D84315] font-medium text-2xl sm:text-3xl lg:text-4xl">
        {prefix}{number}{suffix}
      </p>
      <p className="text-xs sm:text-sm lg:text-base text-gray-600 text-center whitespace-normal">{isYear ? text : `No. of ${text}`}</p>
    </div>
  );
};
...
const Stats = () => {
  return (
    <div className="relative -mt-12 sm:-mt-16 w-full">
      <div className="bg-white rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.1)] py-6 sm:py-8 lg:py-10 px-4 sm:px-6 lg:px-8 border border-gray-100">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 xl:gap-12 justify-items-center w-full">
